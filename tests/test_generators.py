...
def test_filter_by_currency(transaction: list[dict[str, str | dict[str, Any] | int]]):
    ...
    result = list(filter_by_currency(transaction, ""))
    assert result == []
    result = list(filter_by_currency(transaction, "EUR"))
    assert result == []
    result = (filter_by_currency([], ""))
    assert next(result) == "Отсутствуют заданные параметры"
    result = (filter_by_currency({}, ""))
    assert next(result) == "Отсутствуют заданные параметры"
